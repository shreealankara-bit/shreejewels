...
const mapCategory = (c) => ({ ...c, _id: c.id, parentCategory: c.parentCategory ? { ...c.parentCategory, _id: c.parentCategory.id } : null });
...
const getCategories = asyncHandler(async (req, res) => {
  ...
  const subs = await prisma.category.findMany({
    where: { ...where, parentCategoryId: { in: rootIds } },
    orderBy: [{ order: 'asc' }, { name: 'asc' }],
  });
...
const createCategory = asyncHandler(async (req, res) => {
  const { name, description, parentCategory, order, isActive, metaTitle, metaDescription } = req.body;
  if (!name) {
    res.status(400);
...
const updateCategory = asyncHandler(async (req, res) => {
  ...
  const data = {
    ...
    ...(req.body.parentCategory !== undefined ? { parentCategoryId: req.body.parentCategory || null } : {}),
