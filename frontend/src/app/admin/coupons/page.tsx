...
const EMPTY_FORM = { code: '', description: '', discountType: 'percentage', value: '', minOrderAmount: '', maxDiscount: '', expiry: '', usageLimit: '', isActive: true };
...
export default function AdminCouponsPage() {
  ...
  const openEdit = (c: any) => {
    setEditCoupon(c);
    setForm({ code: c.code, description: c.description || '', discountType: c.discountType, value: c.value.toString(), minOrderAmount: c.minOrderAmount.toString(), maxDiscount: c.maxDiscount?.toString() || '', expiry: c.expiry?.slice(0, 10) || '', usageLimit: c.usageLimit?.toString() || '', isActive: c.isActive });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const data = { ...form, value: Number(form.value), minOrderAmount: Number(form.minOrderAmount) || 0, maxDiscount: form.maxDiscount ? Number(form.maxDiscount) : null, usageLimit: form.usageLimit ? Number(form.usageLimit) : null };
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-charcoal-800">
            {loading ? Array.from({ length: 4 }).map((_, i) => (
              <tr key={i} className="animate-pulse"><td colSpan={8} className="px-4 py-4"><div className="h-4 bg-cream-50 rounded w-full" /></td></tr>
            )) : coupons.length === 0 ? (
              <tr><td colSpan={8} className="text-center py-12 text-charcoal-500">No coupons yet</td></tr>
            ) : coupons.map(c => (
